...
Advantages over ETL:
  * Transformations are declarative SQL — easier to audit and version.
  * Raw data is always preserved in Bronze for replay / debugging.
  * Database engine handles joins, aggregates, and type casting.
"""
...
logger = get_logger("elt_pipeline", Config.from_env().log_dir)
...
class ELTPipeline:
    ...
    def run(self) -> Dict[str, int]:
        ...
        metrics = {
            ...
            "gold_refreshed": 4,            # actual count of views refreshed
        }
        logger.info(f"=== ELT Pipeline finished in {elapsed:.1f}s | metrics={metrics} ===")
        return metrics
    ...
    def _transform_silver(self) -> int:
        """Use SQL to unpack bronze JSON into typed silver tables.

        Returns the total number of silver rows affected across all tables.
        """
        with self.store.session() as sess:
            # ── Parts ──────────────────────────────────────────────────
            r = sess.execute(
            ...
            r = sess.execute(
                text("""
                    ...
                    FROM bronze.listings_raw b
                    CROSS JOIN LATERAL jsonb_array_elements((b.raw_json::jsonb)->'shipping'->'options') AS opt
                    LEFT JOIN silver.shipping_options s
                        ON s.ref_no = b.ref_no
            ...
            # ── Similar items (CROSS JOIN LATERAL) ───────────────────
            r = sess.execute(
                text("""
                    ...
                    SELECT
                        ...
                        sim->>'tag'
                    FROM bronze.listings_raw b
                    CROSS JOIN LATERAL jsonb_array_elements((b.raw_json::jsonb)->'similar_items') AS sim
                    LEFT JOIN silver.similar_items s
                        ON s.listing_ref_no = b.ref_no
                        AND s.scraped_at = b.scraped_at
            ...
            r = sess.execute(
                text("""
                    ...
                    SELECT
                        ...
                        img
                    FROM bronze.listings_raw b
                    CROSS JOIN LATERAL jsonb_array_elements_text((b.raw_json::jsonb)->'images') AS img
            ...
            r = sess.execute(
                text("""
                    ...
                    SELECT
                        ...
                        rev->>'text'
                    FROM bronze.listings_raw b
                    CROSS JOIN LATERAL jsonb_array_elements((b.raw_json::jsonb)->'reviews'->'reviews') AS rev
